...
import pygame
from gtts import gTTS                                                                     #packages
from mutagen.mp3 import MP3
from io import BytesIO
mode = input("Select mode:\n1. Repeat mode - a\n2. Sound effects - b\n3. Random answers of question - c: ")
if("a" in mode.lower()):                                                                 #choosing the mode
    pygame.mixer.init()
    ...
    mp3_fp.seek(0)
    pygame.mixer.music.load(mp3_fp, "mp3")                                             #introduction to 1st mode
    ...
    try:
        while(True):
                ...
                with sr.Microphone() as source:
                    print("Say something!")
                    audio = r.listen(source, timeout = 7)                              #listening the user
                ...
                while pygame.mixer.music.get_busy():                                  #repeating it
                    time.sleep(0.1)
...
if("b" in mode.lower()):
    ...
    mp3_fp = BytesIO()
    tts.write_to_fp(mp3_fp)                                                       #intro to 2nd mode
    mp3_fp.seek(0)
    pygame.mixer.music.load(mp3_fp, "mp3")
    pygame.mixer.music.play()
    while pygame.mixer.music.get_busy():
    ...
    try:
        while(True):
            ...
            user = r.recognize_google(audio)                                    #listening the user
            print(user)
            se("cry")
            se("laugh")
            se("horror")
            se("applause")
            se("animal")
            if("random" in user.lower()):                                                  #executing it
...
if("c" in mode.lower()):
    ...
    mp3_fp = BytesIO()
    tts.write_to_fp(mp3_fp)
    mp3_fp.seek(0)                                                                  #intro for 3rd mode
    pygame.mixer.music.load(mp3_fp, "mp3")
    pygame.mixer.music.play()
    while pygame.mixer.music.get_busy():
        time.sleep(0.1)
    ...
    try:
        while True:
            ...
            mp3_fp.seek(0)
            pygame.mixer.music.load(mp3_fp, "mp3")
            pygame.mixer.music.play()                                              #answering it
            while pygame.mixer.music.get_busy():
                time.sleep(0.1)
            pygame.mixer.music.stop()
            pygame.mixer.music.unload()
